...
void ofApp::setup() {
  ...
  /*
    I don't know what most of this does, and I hope to find out
  */
  ...
  ofAddListener(_openVR.ofxOpenVRControllerEvent, this,
                &ofApp::controllerEvent);
  ...
  vertex = "#version 150\n";
  vertex += STRINGIFY(uniform mat4 matrix;
  ...
  fragment += STRINGIFY(uniform sampler2DRect baseTex;
  ...
  vertex += STRINGIFY(uniform mat4 matrix;
                      ...
                      in vec4 position; in vec3 v3ColorIn;
  ...
  fragment += STRINGIFY(in vec4 v4Color; out vec4 outputColor;
                        void main() { outputColor = v4Color; });
...
//--------------------------------------------------------------
void ofApp::exit() {
  ofRemoveListener(_openVR.ofxOpenVRControllerEvent, this,
                   &ofApp::controllerEvent);
...
//--------------------------------------------------------------
void ofApp::update() { _openVR.update(); }

//--------------------------------------------------------------
void ofApp::draw() {
  _openVR.render();

  _openVR.renderDistortion(0, 0, ofGetWidth(), ofGetHeight());
...
void ofApp::render(vr::Hmd_Eye nEye) {
  // OF textured cube
  glm::mat4x4 currentViewProjectionMatrix =
      _openVR.getCurrentViewProjectionMatrix(nEye);
  ...
  if (_openVR.isControllerConnected(vr::TrackedControllerRole_LeftHand)) {
    glm::mat4x4 leftControllerPoseMat =
        currentViewProjectionMatrix *
        _openVR.getControllerPose(vr::TrackedControllerRole_LeftHand);
  ...
  if (_openVR.isControllerConnected(vr::TrackedControllerRole_RightHand)) {
    glm::mat4x4 rightControllerPoseMat =
        currentViewProjectionMatrix *
        _openVR.getControllerPose(vr::TrackedControllerRole_RightHand);
...
void ofApp::controllerEvent(ofxOpenVRControllerEventArgs& args) {
  cout << "ofApp::controllerEvent > role: " << (int)args.controllerRole
       << " - event type: " << (int)args.eventType
       << " - button type: " << (int)args.buttonType
